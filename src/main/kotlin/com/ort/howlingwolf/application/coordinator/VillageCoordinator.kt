...
class VillageCoordinator(
    ...
    fun findParticipant(village: Village, playerId: Int): VillageParticipant? {
        val participant: VillageParticipant? =
            village.participant.memberList.find { it.playerId == playerId && !it.isGone }
    ...
    fun registerVillageFromReservedIfNeeded() {
        val prologueVillages =
            villageService.findVillages(villageStatusList = listOf(VillageStatus(CDef.VillageStatus.プロローグ)))
        // プロローグの村があったら登録しない
        if (prologueVillages.list.isNotEmpty()) return
        // 次に生成される村を取得
        val nextVillage = autogenerateVillageService.findReservedVillages(1).list.first()
        // 開始日時
        val startCandidateDatetime = LocalDateTime.of(LocalDate.now(), nextVillage.startTime)
        val startDatetime =
            if (startCandidateDatetime.isAfter(HowlingWolfDateUtil.currentLocalDateTime())) startCandidateDatetime
            else startCandidateDatetime.plusDays(1L)
        // 登録
        val village = registerVillage(
            paramVillage = Village.createAutoGeneratedForRegister(
                playerId = 1, // masterアカウントで作成
                organization = nextVillage.organization,
...
//        tweetRepository.tweet(village.createNewVillageMessage())
    ...
    fun findActionSituation(
        ...
        val skillRequest: SkillRequest? =
            if (participant == null) null else village.participant.member(participant.id).skillRequest
